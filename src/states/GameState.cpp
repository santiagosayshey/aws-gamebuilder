...
GameState::GameState(sf::RenderWindow& window, const GameSettings& settings, const std::vector<PlayerBet>& initialBets)
    ...
    , hitButton(sf::Vector2f(0.f, 0.f), sf::Vector2f(150.f, 50.f), "HIT", sf::Font())
    , standButton(sf::Vector2f(0.f, 0.f), sf::Vector2f(150.f, 50.f), "STAND", sf::Font())
    , doubleDownButton(sf::Vector2f(0.f, 0.f), sf::Vector2f(200.f, 50.f), "DOUBLE DOWN", sf::Font())
    , wildcardButton(sf::Vector2f(0.f, 0.f), sf::Vector2f(200.f, 50.f), "WILDCARD", sf::Font())
{
    // Load font
                ...
                if (roundInProgress && !roundConcluded) {
                    ...
                    if (hitButton.isMouseOver(mousePos)) {
                        if (current.hasForesight()) {
                            Card c = deck.peek();
                            messageText.setString("Next card is " + c.toString() + ". Click hit again to confirm.");
                        ...
                        if (current.canBet(currentBet)) {  // Using existing canBet method
